...
async function login(username, password) {
	return new Promise(async (resolve, reject) => {
		try {
			...
			const token = await mysqlQuery(
				`select token from users where token = '${newToken}'`
			)
			if (token.length > 0) {
...
app.post('/account/register', (req, res) => {
	...
	if (!validadeUsername(username)) {
		res.json(
			error(
				'Username must have only letter and numbers and be between 4 and 20 characters'
			)
		)
		return
	}
...
app.get('/puzzle/random', (req, res) => {
	res.json(
		randomPuzzle(req.query.minRating, req.query.maxRating, req.query.themes)
	)
})
...
sockets.on('connection', (socket) => {
	...
	socket.on('create-room', ({ time, rated, isPublic }) => {
		...
		if (time < 10) {
			socket.emit(
				'create-room',
				'error:Time must be greater than 10 seconds'
			)
			return
		}
		if (time > 10800) {
...
function Game(id, time, rated = false, isPublic = false) {
	...
	function win(color) {
		if (players[color].token && rated) {
			mysqlQuery(
				`update users set elo = elo + 10 where token = '${players[color].token}'`
			)
	...
	function validMove() {
		...
		sockets
			.to(id + '-spectator')
			.emit('move', new Move(from.x, from.y, to.x, to.y))
	...
	async function join(socket, token, color) {
		...
		if (
			token &&
			(players.white.token === token || players.black.token === token)
		) {
		...
		if (state === 0) {
			if (!color) {
				...
				if (players[color].socket === null) {
					...
					players[oppositeColor(color)].timer = new Timer(
						secToMs(gameTime)
					)
